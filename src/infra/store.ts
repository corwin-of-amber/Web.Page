...
abstract class StoreBase<Doc = any> {
    abstract load(): Doc
    abstract save(p: Doc): void

    /**
     * Persists value to storage before app exists.
     */
    autosave(d: Doc) {
    ...
    /**
     * Loads document, and sets returned value to autosave.
     * (document should be mutable for this to make sense.)
     * @param init value to use if key is uninitialized.
     * @param assign treat `init` as document to be updated
     *   with loaded data.
     */
    autoload(init: Doc = {} as Doc, assign: boolean = true) {
        let d = this.load() ?? init;
        if (assign) d = Object.assign(init, d);
        this.autosave(d);
        return d;
    }
}

class LocalStore<Doc = any> extends StoreBase<Doc> {
    ser: Serialization
    key: string
    loadedValue: string
...
class FileStore<Doc = any> extends StoreBase<Doc> {
    ser: Serialization
    filename: string
    loadedValue: string
